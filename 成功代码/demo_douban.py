...
# 记录本次爬虫程序启动的时间
spider_start = str(datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
spider_start_log = str(datetime.datetime.now().strftime("%Y年%m月%d日:%H-%M-%S"))
...
if tag in movie_type:
    ...
    # 将获取到的数据储存在content变量中
    content = response.text

    # 将content中的数据转化为json数据
    ...
    for item in content_json["subjects"]:
        ...
        rate = item["rate"]
        url = item["url"]
        movie = u"电影名称：%s，评分：%s，电影详情地址:%s\n" % (title, rate, url)
        print(movie)

        # 设置保存的文件名
        file_name = tag.decode("utf-8").encode("gbk") + "_" + spider_start + ".txt"
    ...
    # file_size = file_size / float(1024 * 1024) # MB
    file_size = file_size / float(1024)  # KB
    file_size = round(file_size, 2)
    file_name_for_log = tag + "_" + spider_start + ".txt"

    log_msg = "爬虫程序在%s,保存了文件：%s,文件大小：%sKB,用时：%s秒\n" % (spider_start_log, file_name_for_log, file_size, use_time)
